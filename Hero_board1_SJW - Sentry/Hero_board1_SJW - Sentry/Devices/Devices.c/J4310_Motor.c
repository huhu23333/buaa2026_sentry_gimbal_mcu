 ...
 * @retval None
 */
static float uint_to_float(uint16_t X_int, float X_min, float X_max, int Bits) // uint写成int是什么意思
{
...
void J4310_setParameter(float uq1, float uq2, float uq3, float uq4, float uq5, uint8_t *data)
{
  ...
  volatile uint16_t debug_value = Postion_Tmp;
...
void J4310_getInfo(Can_Export_Data_t RxMessage)
{
  ...
  volatile uint16_t debug_value = J4310_Array[StdId]->angleInit;
