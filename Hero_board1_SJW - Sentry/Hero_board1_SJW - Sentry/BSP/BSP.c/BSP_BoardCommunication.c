...
void Board1_To_2(void)
{
  ...
  data[6] = ControlMes.yaw_velocity >> 8;//小YAW模式下，底盘不采用数据。大YAW模式下，为大YAW的目标速度。
  ...
  data2_Fun[3] |= (uint8_t)(ControlMes.AutoAimFlag & 0x01) << 1;//自瞄开关
  data2_Fun[3] |= (uint8_t)(ControlMes.change_Flag & 0x01) << 2;//变速小陀螺开关
