...
void MX_FREERTOS_Init(void) {
  ...
  Task_VOFA_Handle = osThreadCreate(osThread(Task_VOFA_Handle), NULL);
	...
	osThreadDef(Task_J4310_onlineCheck_Handle, J4310_onlineCheck, osPriorityHigh, 0, 128);
...
void ALL_Init(void const * argument)
{
  ...
  for (;;)
  {
    ...
    Position_PIDInit(&M6020s_YawIPID, yaw_I_p, yaw_I_i, yaw_I_d, 0, 30000, 10000 , 10000);
    ...
    Cloud_Init();	
    if(RemoteController == "DT7")
    {
      DT7_Init(); // 大疆DT7遥控器
    }
    if(RemoteController == "TDF")
    {
      TDF_Init();
    }
