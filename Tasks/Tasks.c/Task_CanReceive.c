...
void Can1Receives(void const *argument)
{
  ...
  for (;;)
  {
    xQueueReceive(CAN1_ReceiveHandle, &Can_Export_Data, portMAX_DELAY); // 阻塞式等待CAN1接收数据
...
void Can2Receives(void const *argument)
{
  ...
  for (;;)
  {
    ...
    else if (ID == CAN_ID_JUDGESYSTEM)
    {
      Board1_FUN.Board1_getJudgeSystemInfo(Can_Export_Data);
    }
